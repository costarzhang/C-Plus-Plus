...
typedef struct VertAGraph { // 逆邻接表
    int v;                  // 顶点数
    int e;                  // 边数
    VNode *adjlist;         // 顶点表
    float **edges;          // 逆邻接矩阵
...
int *topsort(AGraph agraph) {
    ...
    while (top != -1) {
        ...
        while (p != nullptr) {
            ...
            if (agraph.adjlist[j].incount == 0) { // 将入度变为0的结点入栈
                stack[++top] = j;
            }
            p = p->nextarc;
        }
...
int topsort1(AGraph agraph) {
    ...
    while (rear != front) {
        ...
        while (p != nullptr) {
            j = p->adjvex;
            --(agraph.adjlist[j].incount);
            if (agraph.adjlist[j].incount == 0) { // 将入度变为0的结点入队
                rear = (rear + 1) % (agraph.v + 1);
                queue[rear] = j;
            }
            p = p->nextarc;
...
int *verttopsort (VertAGraph vagraph) {
    ...
    while (top != -1) {
        ...
        while (p != nullptr) {
            ...
            if (vagraph.adjlist[j].outcount == 0) {// 将出度为一的顶点入栈
                stack[++top] = j;
            }
            p = p->nextarc;
        }
...
int *ve_k(int top[], VertAGraph vagraph) {
    ...
    for (int i = 1; i <= vagraph.v - 1; i++) { // 更具拓扑序列，依次求剩余结点的ve
...
int *vl_k(int top[], AGraph agraph, int ve) {
    ...
    for (int i = agraph.v -2; i >=0; i--) { // 更具拓扑序列，依次求剩余结点的ve
...
void key_route(map<string,int> &e, map<string,int> l, AGraph &agraph) {
    ...
    for (loop = e.begin(); loop != e.end();) {
        if (loop->second == l[loop->first]) { // 检查活动最早开始时间和最迟开始时间是否相等
            loop++;
        } else {
